...
A *Carnot cycle* is an idealized process that is composed of two isothermal and two adiabatic transformations. Each transformation is either an expansion or a compression of an *ideal gas*. All transformations are assumed to be *reversible* and no energy is lost to mechanical friction.
...
```{r FigCarnotPV, echo=FALSE, fig.align='center', fig.cap='PV-Diagram of Carnot Cycle'}
x <- seq(0,1.25,0.01)
i <- 0.2
f <- 1
p1 <- 0.5
...
where we denote $\left| Q_h \right|$ the absolute value of the heat that gets **into** the system from the hot reservoir. 
...
where the minus sign in front of the work is necessary because the efficiency is defined as a positive number. Replacing eq. \@ref(eq:WtotCC) into \@ref(eq:effCC1), we obtain:

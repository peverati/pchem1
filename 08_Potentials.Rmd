...
| **Helmholtz Energy** |     $A$    | $dA=-SdT-PdV+\sum_i\mu_i dn_i$ |      $T,V,\{n_i\}$      |
| **Gibbs Energy**     |     $G$    |  $dG=VdP-SdT+\sum_i\mu_i dn_i$ |      $T,P,\{n_i\}$      |
\normalsize
...
- Helmholtz energy ($A$) is the capacity to do mechanical plus non-mechanical work.^[For the mathematically inclined, an entertaining method to summarize the same thermodynamic potentials is the [thermodynamic square](https://en.wikipedia.org/wiki/Thermodynamic_square).]
...
## Gibbs and Helmholtz Energies {#freeenergies}
...
we can then simplify the definition of each energy, eqs. \@ref(eq:dadef2) and \@ref(eq:dgdef2):
...
- During a spontaneous process at constant *temperature* and *volume*, the *Helmholtz energy* will decrease $(dA<0)$, until it reaches a stationary point at which the system will be at equilibrium $(dA=0)$.
- During a spontaneous process at constant *temperature* and *pressure*, the *Gibbs energy* will decrease $(dG<0)$, until it reaches a stationary point at which the system will be at equilibrium $(dG=0)$.

> ```

```{r agfig, echo=FALSE, fig.align='center', fig.cap='Behavior of Helmholtz (red) and Gibbs (blue) Energies for Spontaneous Processes at Constant $T,V$ (left) and Constant $T,P$ (right).'}
x <- seq(0,1.25,0.01)
i <- 0.2
f <- 1
p1 <- 0.5
...
plot(x, 2/x^2+25,
     ...
     yaxt='n',
     mgp=c(0.5,0.5,0),
     ylab = "Helmholtz Energy", xlab = "time", col = "red")
...
plot(x, 2/x^2+25,
     ...
     yaxt='n',
     mgp=c(0.5,0.5,0),
     ylab = "Gibbs Energy", xlab = "time", col = "#2E9FDF")
arrows(-10 , 25, 10, 25 , length=0, angle=90, code=3, lty="dotted")
...
**3. Which of the following formulas is the fundamental equation for the Helmholtz energy?**
...
c. Gibbs energy is the capacity to do mechanical work only.
d. Helmholtz energy is the capacity to do mechanical plus non-mechanical work.
...
**5. What are the natural variables of the Gibbs energy?**

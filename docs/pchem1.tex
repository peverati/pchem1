...
The concentration of each of the species can then be plot against time, obtaining the three different behaviors reported in Figure XXX. An important relationship that can be derived from eq. \eqref{eq:kincomp2} is that:
...
As can be seen from the plots in Figure XXX, after a sufficiently long time, the systems reach a dynamic equilibrium, where the concentration of \(\mathrm{A}\) and \(\mathrm{B}\) don't change. These equilibrium concentrations can be calculated replacing \(t=\infty\) in eq. \eqref{eq:kinopp1}:
...
Initial rate \(\left(\frac{M}{s}\right)\)\strut
\end{minipage}\tabularnewline
\midrule
\endhead

...
=======
\date{31 August 2020}
>>>>>>> parent of 0e96fae... Fixing pdf, hopefully...
...
\hypertarget{free-expansion-at-constant-temperature-xxx-merge-with-next-and-change-title-and-figure-xxx}{%
\subsection{Free expansion at constant temperature XXX merge with next and change title and figure XXX}\label{free-expansion-at-constant-temperature-xxx-merge-with-next-and-change-title-and-figure-xxx}}
...
\hypertarget{isothermal-expansion-against-a-constant-external-pressure-xxx-remove-this-and-replace-with-compression.-also-show-the-difference-on-the-pv-plot-xxx}{%
\subsection{Isothermal expansion against a constant external pressure XXX Remove this and replace with compression. Also show the difference on the PV-plot XXX}\label{isothermal-expansion-against-a-constant-external-pressure-xxx-remove-this-and-replace-with-compression.-also-show-the-difference-on-the-pv-plot-xxx}}
...
\begin{equation}
\left| W_{\text{1-step}} \right| = P_{\text{ext}} (V_f-V_i) = P_{\text{ext}} \Delta V.
  \label{eq:Warea1}
\end{equation}
...
\begin{equation}
\left| W_A \right| = P_1 (V_1-V_i) = P_1 \Delta V_A.
  \label{eq:Warea2}
\end{equation}
...
\begin{equation}
\left| W_B \right| = P_f (V_f-V_1) = P_{\text{ext}} \Delta V_B.
  \label{eq:Warea3}
\end{equation}
...
\begin{equation}
  \left| W_{\text{2-step}} \right| = \left| W_A \right| + \left| W_B \right| = P_1 \Delta V_A+P_f \Delta V_B.
  \label{eq:Warea4}
\end{equation}

As can be easily verified by comparing the shaded areas in the plots, \(\left| W_{\text{2-step}} \right| > \left| W_{\text{1-step}} \right|\).

We can easily extend this procedure to consider processes that happens in 3, 4, 5, \ldots, \(n\) steps:

\begin{equation}
  \left| W_{\text{n-step}} \right| = \sum_n P_n \Delta V_n.
  \label{eq:Wareasum}
\end{equation}

What is the limit of this procedure? In other words, what happens when \(n \rightarrow \infty\)? A simple answer is given by the plots in the next Figure, which clearly demonstrates that the maximum value of the area underneath the curve \(\left| W_{\text{max}}\right|\) is achieved in an \(\infty\)-step process, for which the work is calculated as:

\begin{equation}
  \left| W_{\infty \text{-step}} \right| = \left| W_{\text{max}} \right| = \lim_{\Delta V_n\rightarrow 0} \sum_n^\infty P_n \Delta V = \int_{i}^{f} PdV.
...
How do we solve the integral in eq. \eqref{eq:WintsolvedV2}? The pressure in this process is not constant since it changes at each microscopic step throughout the process. Therefore \(P\) cannot be moved outside the integral, as we did in the previous chapter for eq. \eqref{eq:Wint3}. However, if our gas is ideal, we can replace \(P\) using the ideal gas law \(P=\frac{nRT}{V}\), and solve the integral considering that \(n\), \(R\), and \(T\) are constant:

\begin{equation}
  W_{\infty \text{-step}} = - nRT \int_{i}^{f} \frac{dV}{V} = -nRT \ln \frac{V_f}{V_i},
...
\begin{equation}
  W_{\infty \text{-step}} = -nRT \ln \frac{P_i}{P_f}.
  \label{eq:WintsolvedP}
\end{equation}

Comparing eq. \eqref{eq:Warea1} to eq. \eqref{eq:WintF} for two processes with the same values of initial and final volume shows how different the values of the work associated with them can be. This fact explains why work is a path function. If we perform a fast 1-step expansion, the system will perform an amount of work that is much smaller than the amount of work it can perform if the expansion between the same points happens slowly in an \(\infty\)-step process.

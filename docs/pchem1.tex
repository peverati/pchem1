...
Non-ideal gases (sometimes also referred to as ``real gases''), do not behave as ideal gases because one or more of the assumptions XXX are violated. To study how their behavior differs from an ideal gas we can use either a \(P\) \emph{vs.} \(V\) diagram, or a \(PV\) \emph{vs.} \(P\) diagram:
...
The \emph{Joule--Thomson coefficient}, \(\mu_{\mathrm{JT}}\), measures the rate of change of temperature of a gas with respect to pressure in the Joule--Thomson process:
...
The critical temperature is the coordinate of a unique point, called the \textbf{critical point}, that can be visualized in the three-dimensional \(T,P,V\) diagram of each gas (Figure \ref{FigTPVd}\footnote{This diagram is taken from \href{https://en.wikipedia.org}{Wikipedia} and distributed under CC-BY-SA license.}).
...
\begin{equation}
z_c=\frac{P_c V_{m,c}}{R T_c}=\left( \frac{a}{27b^2} \right) \left( \frac{3b}{R} \right) \left( \frac{27Rb}{a} \right) = \frac{3}{8} = 0.375,
\label{eq:critiz}
\end{equation}

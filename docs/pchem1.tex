...
The concept of an ideal gas is a theoretical construct that allows for simple treatment and interpretation of the behavior of gases. As such, the ideal gas is a simplified \emph{model} that we use to understand nature, and it does not correspond to any real system. The following two assumptions define the ideal gas model:

\begin{quote}
\begin{definition}
\protect\hypertarget{def:iedalgas}{}{\label{def:iedalgas} }

\begin{itemize}
\tightlist
\item
  The molecules that compose an ideal gas do not occupy any volume.
\item
  The molecules that compose an ideal gas do not interact with each other.
\end{itemize}
\end{definition}
\end{quote}
...
which results in a the familiar \(PV\) plots of Figure XXX. As we already discussed in chapter \ref{ZerothLaw}, each of the curves in Figure XXX is obtained at constant temperature, and it is therefore called \emph{``isotherm.''}
...
It took scientists more than a century to expand Boyle's work and study the relationship between volume and temperature. In 1787 Jacques Alexandre César Charles (1746--1823) wrote the relationship that is now known as Charles's Law:
...
The interesting thing about isobars is that each line seems to converge to a very specific point along the temperature line when we extrapolate them to \(V\rightarrow 0\). This led to the introduction of the absolute temperature scale, suggesting the fact that the temperature will never get smaller than \(-273.15^\circ\mathrm{C}\).

It took an additional 21 years to write a formal relationship between pressure and temperature. The following relationships were proposed Joseph Louis Gay-Lussac (1778--1850) in 1808:
...
Ten years later, Amedeo Avogadro (1776--1856) discovered a seemingly unrelated principle by studying the composition of matter. His Avoadro's Law encodes the relationship between the number of moles in an ideal gas and its volume as:
...
Despite all of the ingredients being available since more than 20 years, it's only in 1834 that Benoît Paul Émile Clapeyron (1799--1864) was finally able to combine all of them into what is now known as the \textbf{ideal gas Law}. Using the same formulas obtained above, we can write:
...
Non-ideal gases (sometimes also referred to as ``real gases''), do not behave as ideal gases because one or more of the assumptions in XXX are violated. To study how their behavior differs from an ideal gas we can use either a \(P\) \emph{vs.} \(V\) diagram, or a \(PV\) \emph{vs.} \(P\) diagram:

\hypertarget{the-van-der-waals-equation}{%
\subsection{The van der Waals equation}\label{the-van-der-waals-equation}}

What characterize non-ideal gases is the fact that there is no unique equation that we can use to describe their behavior. For this reason, we have a plethora of several experimental equations, none of which is superior to the other. THe simplest of the experimental equations was obtained in 1873 by Johannes Diderik van der Waals (1837--1923), and it's the only equation that we are going to analyze in details here because of its simple interpretation. However, the van der Waals (vdW) equation is far from being universal, and for several non-ideal gases it is severely inaccurate.

The vdW equation includes two empirical parameters (\(a\) and \(b\)) that assume different values for each non-ideal gas. Each of the parameter corresponds to a correction for the breaking of the two conditions that define the ideal gas behavior (definition \ref{def:idealgas}). The vdW equation is obtained from the following simple substitutions to the ideal gas equation:
...
We have already met William Thomson, also known as Lord Kelvin, and his seminal work on the second law of thermodynamics. In conjunction with that work, Thomson is famous for having developed a sensitive method for measuring the temperature changes related to the expansion of a gas. These experiments improved on the earlier work by James Joule, and Lord Kelvin's improved instrument depicted in Figure \ref{fig:FigJT} is named the Joule--Thomson apparatus. The apparatus is composed of two chambers, each with its own mobile piston. The chambers are connected via a valve or porous plug. The entire apparatus is also thermally isolated from the surroundings. This instrument is a more sensitive version of the Joule expansion apparatus that we already described in section \ref{FirstLaw} (compare with Figure \ref{fig:FigJexp}).

Thomson realized that a gas flowing through an obstruction experience a drop in pressure. If the entire apparatus is insulated, it will not exchange heat with its surroundings (\(Q=0\)), and each transformation will happen at adiabatic conditions. Let's consider an initial condition with 1 mol of gas in the left chamber, occupying a volume \(V_l\), and a completely closed left chamber, for which \(V_r^i=0\). After the process completes, the volume of the right chamber will redice to \(V_l^f=0\), while the volume of the right chamber will be \(V_r\). Using the first law of thermodynamics, we can write:
...
It is evident from eq. \eqref{eq:comprdef} that the compressibility factor is dependent on the pressure, and for an ideal gas \(z=1\) always. For a non-ideal gas at any given pressure, \(z\) can be higher or lower than one, separating their behaviors into two possibilities. The dependence of the compressibility factor against pressure is represented for \(\mathrm{H}_2\) and \(\mathrm{CO}_2\) in Figure \ref{fig:FigZb}.

...
We can easily extend this procedure to consider processes that happens in 3, 4, 5, ..., $n$ steps:
...
How do we solve the integral in eq. \@ref(eq:WintsolvedV2)? The pressure in this process is not constant since it changes at each microscopic step throughout the process. Therefore $P$ cannot be moved outside the integral, as we did in the previous chapter for eq. \@ref(eq:Wint3). However, if our gas is ideal, we can replace $P$ using the ideal gas law $P=\frac{nRT}{V}$, and solve the integral considering that $n$, $R$, and $T$ are constant:
